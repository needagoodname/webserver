...
void Log::init( int level = 1, const char* path, const char* suffix,
    ...
    snprintf(fileName, LOG_NAME_LEN - 1, "%s/%04d_%02d_%02d%s", 
            path_, t.tm_year + 1900, t.tm_mon + 1, t.tm_mday, suffix_); //存储路径写入filename
    toDay_ = t.tm_mday;

    {
        ...
        if(fp_ == nullptr) {
            mkdir(path_, 0777); // 创建名为path_, 权限为0777的目录
            fp_ = fopen(fileName, "a");
        } 
        assert(fp_ != nullptr);
    }
...
void Log::WriteLog(int level, const char *format, ...) {
    ...
    if (toDay_ != t.tm_mday || (lineCount_ && (lineCount_  %  MAX_LINES == 0)))
    {
        ...
        if (toDay_ != t.tm_mday) // 时间不匹配更改日志名
        {
    ...
    // 生成日志信息
    {
        ...
        if(isAsync_ && deque_ && !deque_->full()) {
            ...
            deque_->push_back(buff_.RetrieveAllAsString()); // 异步加入阻塞队列
        } else {
            std::lock_guard<std::mutex> lock(mtx_);
            fputs(buff_.Peek(), fp_); // 同步直接写入
        }
        {
            std::lock_guard<std::mutex> lock(mtx_);
            buff_.RetrieveAll();
